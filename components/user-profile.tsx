...
import { Heart, MessageCircle, Bookmark, Award, TrendingUp, User } from "lucide-react"
        ...
        <CardContent className="p-8">
          <div className="flex items-start space-x-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-600 mb-4">Member since {userData.joinedDate}</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-gray-900">{userData.totalReactions}</div>
                  <div className="text-sm text-gray-600">Reactions</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-gray-900">{userData.totalBookmarks}</div>
            ...
            <div className="space-y-4">
              {categoryRankings.map((ranking, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <h3 className="font-medium text-gray-900">{ranking.category}</h3>
                    <p className="text-sm text-gray-600">
                      {ranking.posts} posts • {ranking.reactions} reactions
                    </p>
                  </div>
                  <div className="text-right">
                    <div className="text-lg font-bold text-gray-900">#{ranking.rank}</div>
                  </div>
                </div>
              ))}
            </div>
              ...
              {topPosts.map((post, index) => (
                <div key={post.id} className="p-3 bg-gray-50 rounded-lg">
                  ...
                  <Link href={`/post/${post.id}`}>
                    <h3 className="font-medium text-gray-900 hover:text-gray-700 cursor-pointer mb-2">{post.title}</h3>
                  </Link>
      ...
      <Card>
        ...
        <CardContent>
          <div className="h-64 bg-gray-100 rounded-lg flex items-center justify-center">
            <p className="text-gray-500">Ranking trend graph would be displayed here</p>
          </div>
        </CardContent>
      </Card>
    </div>
