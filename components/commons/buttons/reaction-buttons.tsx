...
export function BaseReactionButtons({
  reactions,
  myReaction = null,
  onReactionClick,
  ...
  const handleReactionClick = (reactionType: ReactionLabelType) => {
    if (executeWithAuth) {
      executeWithAuth(() => onReactionClick(reactionType));
    } else {
...
// withAuth로 감싼 인증이 필요한 리액션 버튼
export const ReactionButtons = withAuth(BaseReactionButtons);
